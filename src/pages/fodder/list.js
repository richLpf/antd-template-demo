...
function List(props){
    ...
    return <Fragment>
        <Card
        className="fodder-box"
        style={{ textAlign: "left" }}
      >
        <Row gutter={16}>
          {fodderList.map((item, index) => {
            return (
              <Col span={4} key={index} style={{ marginBottom: 10 }} onClick={()=>props.handleSelect(item)}>
                <Card
                  size="small"
                  ...
                  cover={
                    <div className="fodder-top">
                      ...
                      <div className="shadow" style={{background: fodderColorList[item.status]}}>
                ...
                >
                  <Meta title={
                    <div style={{textAlign: 'center', width: '100%', fontSize: 14 }}>{item.title}</div>
                  } />
                </Card>
              </Col>
            );
